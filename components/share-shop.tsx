...
export function ShareShop({ shopId, shopName }: ShareShopProps) {
  ...
  const shareOptions = [
    ...
    },
    {
      ...
      action: () => {
        window.open(`https://wa.me/?text=Check out ${shopName}! ${shopUrl}`, '_blank')
      },
    ...
    },
    {
      ...
      action: () => {
        window.open(`https://twitter.com/intent/tweet?text=Check out ${shopName}!&url=${encodeURIComponent(shopUrl)}`, '_blank')
      },
      ...
      <AnimatePresence>
        {isOpen && (
          <>
            ...
            >
              <h4 className="font-semibold text-foreground mb-3">Share {shopName}</h4>
                ...
                {shareOptions.map((option) => (
                  ...
                  >
                    <div className={`${option.className}`}>
                      {typeof option.icon === 'function' ? <option.icon /> : <option.icon className="h-5 w-5" />}
                    </div>
